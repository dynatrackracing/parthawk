...
class AutoService {
  ...
  async getDistinctList({ constraints }) {
    ...
    return this.cacheManager.get(key, async () => {
      // get the distinct on the field passed in
      const statement = Auto.query()
        // distinct fields only
        .distinct('id', 'year', 'make', 'model', 'trim', 'engine')
        // only get those cars that have parts attached to them
        .innerJoin('AutoItemCompatibility', 'Auto.id', 'AutoItemCompatibility.autoId');
