...
const Table = ({ data, columns, setSelectedRows }) => {
  const {
    getTableProps,
    getTableBodyProps,
    headerGroups,
            ...
            {headerGroups.map((headerGroup) => (
             <tr className="text-md font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600" {...headerGroup.getHeaderGroupProps()}>
                {headerGroup.headers.slice(0, 1).map((column) => (
                  <th className="px-4 py-3 text-base" {...column.getHeaderProps(column.getSortByToggleProps())}>
              ...
              return (
                <tr className="text-gray-700" {...row.getRowProps()}>
                  {row.cells.slice(0, 1).map((cell) => {
                    return (
                      <td className="px-4 py-3 border" {...cell.getCellProps()}>{cell.render("Cell")}</td>
                    );
                  })}
                  <td className="px-4 py-3 border">{index + (10 * pageIndex + 1)}</td>
